...
const publicActive=new Map<string,{kind:string;expires:number}>();
...
function acquireLocalPublic(sid:string,ip:string,match:string,kind:string){
 ...
 const locks=['board:'+sid+':'+match,'session:'+kind+':'+sid];
 if(locks.some(key=>publicLocks.has(key)))return {release:noop,error:'Another request is finishing for this session. Retry shortly.',status:429,code:'rate_limit'};
 ...
 const lease=randomUUID();for(const key of locks)publicLocks.set(key,{lease,expires:now+60000});publicActive.set(lease,{kind,expires:now+60000});
